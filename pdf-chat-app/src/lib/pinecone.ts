...
// Updated function to search only within a specific document
export async function searchSimilarChunks(
  queryEmbedding: number[],
  documentId: string, // NEW: Filter by specific document
  limit: number = 5
): Promise<Array<{
  id: string;
  content: string;
  ...
  try {
    ...
    console.log(`Searching for similar chunks in document: ${documentId}`);
    ...
    const queryResponse = await index.query({
      ...
      includeValues: false,
      filter: {
        documentId: { $eq: documentId } // Filter by specific document only
      }
    });
...
export async function deleteDocumentChunks(documentId: string): Promise<void> {
  try {
    ...
    const queryResponse = await index.query({
      vector: new Array(1024).fill(0), // Dummy vector (updated to 1024)
      topK: 10000,
      filter: { documentId: { $eq: documentId } },
      includeMetadata: false,
      includeValues: false,
